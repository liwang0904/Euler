...
//    println(fraction.compareTo(other))
    val fractions = allFractions(8).distinct()
//    for (fraction in allFractions(12000).distinct()) {
//        fractions.add(mutableListOf(fraction.numerator, fraction.denominator))
//        println(fractions)
//    }
    for (fraction in fractions) {
        if (fraction > Fraction(1, 3) && fraction < Fraction(1, 2)) {
            println("numerator = ${fraction.numerator} & denominator = ${fraction.denominator}")
        }
    }
}
...
fun allFractions(max_denominator: Int): List<Fraction> {
    ...
    for (denominator in 2..max_denominator) {
        println(denominator)
        for (numerator in 1..denominator/2) {
